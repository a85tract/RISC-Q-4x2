"""Host-side unit tests for the QuantumModel ADC-seam models (riscq_sim.models). Deterministic,
...
from riscq_sim import models

M = SocMap(SocParams.load(Path(__file__).resolve().parents[3] / "gateware" / "configs" / "sim-2q.json"))
...
M2Q = SocMap(SocParams.load(Path(__file__).resolve().parents[3] / "gateware" / "configs" / "sim-2q1c.json"))
