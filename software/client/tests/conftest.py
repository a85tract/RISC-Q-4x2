...
SW_ROOT = Path(__file__).resolve().parents[1]            # software/client
REPO_ROOT = SW_ROOT.parents[1]                           # the repo
CONFIGS = REPO_ROOT / "gateware" / "configs"             # SoC parameter JSONs live with the RTL
SIM_BUILD = REPO_ROOT / "sim" / "build"                  # verilated models
sys.path.insert(0, str(REPO_ROOT / "sim"))              # riscq_sim, the co-sim package
...
def cosim(request):
    ...
    drv = server.start(CONFIGS / "sim-2q.json", SW_ROOT.parents[1] / "sim" / "build" / "sim-2q")
...
def cosim_2q1c(request):
    ...
    drv = server.start(CONFIGS / "sim-2q1c.json", SW_ROOT.parents[1] / "sim" / "build" / "sim-2q1c")
    m = SocMap(SocParams.from_json(drv.sim.get_params()))
    yield drv, m
    server.stop(drv)
...
def cosim_4x2(request):
    ...
    drv = server.start(CONFIGS / "rfsoc4x2-1q.json", SW_ROOT.parents[1] / "sim" / "build" / "rfsoc4x2-1q")
    m = SocMap(SocParams.from_json(drv.sim.get_params()))
    yield drv, m
    server.stop(drv)
