...
PARAMS = SocParams.load(Path(__file__).resolve().parents[3] / "gateware" / "configs" / "zcu216-14q.json")
M = SocMap(PARAMS)
...
class _StubDrv:                                        # socmap(drv) reads drv.sim.get_params()
    class sim:
        @staticmethod
        def get_params():
            return (Path(__file__).resolve().parents[3] / "gateware" / "configs" / "sim-2q.json").read_text()
